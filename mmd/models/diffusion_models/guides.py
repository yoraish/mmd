...
class GuideManagerTrajectoriesWithVelocity(nn.Module):
    ...
    def __init__(self, dataset, cost, clip_grad=False, clip_grad_rule='norm', max_grad_norm=1., max_grad_value=0.1,
                 ...
                 **kwargs):
        super().__init__()
        self.cost = cost  # CostComposite. Will be complemented with the Cost objects in extra_costs.
        self.dataset = dataset
        ...
        # Extra costs. These are extra cost components that will be evaluated alongside those in self.cost.
        self.extra_cost_l = []
        self.extra_costs_grad_weight_l = []

    def forward(self, x_normalized):  # This is called by the model when asking for guidance.
        ...
        with torch.enable_grad():
            ...
            self.cost.cost_l.extend(self.extra_cost_l)
            self.cost.weight_cost_l.extend(self.extra_costs_grad_weight_l)

            # compute costs
            # append the current velocity trajectory to the position trajectory only for non-interpolated trajectories
            # self.cost calls the eval() method for each guide in the cost function.
            cost_l, weight_grad_cost_l = self.cost(x, x_interpolated=x_interpolated, return_invidual_costs_and_weights=True)

            self.cost.cost_l = self.cost.cost_l[:len(cost_l) - len(self.extra_cost_l)]
            self.cost.weight_cost_l = self.cost.weight_cost_l[:len(weight_grad_cost_l) - len(self.extra_costs_grad_weight_l)]
    ...
    def add_extra_costs(self, extra_costs, extra_costs_grad_weights):
        self.extra_cost_l.extend(extra_costs)
        self.extra_costs_grad_weight_l.extend(extra_costs_grad_weights)
